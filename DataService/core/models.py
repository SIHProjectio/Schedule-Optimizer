...
class OperationalHours(BaseModel):
    ...
    @classmethod
    def from_config(cls, config_dict: dict) -> "OperationalHours":
        """Create from configuration dictionary"""
        from datetime import time as dt_time
        
        # Parse start time
        start_h, start_m = map(int, config_dict['start_time'].split(':'))
        start_time = dt_time(start_h, start_m)
        
        # Parse end time
        end_h, end_m = map(int, config_dict['end_time'].split(':'))
        end_time = dt_time(end_h, end_m)
        
        # Parse peak hours
        peak_hours = []
        for period in config_dict['peak_hours']:
            start_h, start_m = map(int, period['start'].split(':'))
            end_h, end_m = map(int, period['end'].split(':'))
            peak_hours.append((dt_time(start_h, start_m), dt_time(end_h, end_m)))
        
        return cls(
            start_time=start_time,
            end_time=end_time,
            peak_hours=peak_hours,
            peak_frequency_minutes=config_dict['peak_frequency_minutes'],
            off_peak_frequency_minutes=config_dict['off_peak_frequency_minutes']
        )
