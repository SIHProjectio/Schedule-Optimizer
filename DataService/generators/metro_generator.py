...
class MetroDataGenerator:
    ...
    def __init__(
        ...
        Args:
            num_trains: Number of trains in fleet
            num_stations: Number of stations on route
        ...
        route_config = self.config.get_route_config()
        self.num_stations = min(num_stations, len(route_config['stations']))
        
        self.trainset_ids = [f"TS-{str(i+1).zfill(3)}" for i in range(num_trains)]
    
    @property
    def STATIONS_ALUVA_PETTAH(self) -> List[str]:
        """Get station list from config"""
        route_config = self.config.get_route_config("Aluva-Pettah Line")
        return route_config['stations']
    
    @property
    def DEPOT_BAYS(self) -> List[str]:
        """Get depot bays from config"""
        depot_config = self.config.get_depot_config()
        return depot_config['stabling_bays']
    
    @property
    def IBL_BAYS(self) -> List[str]:
        """Get IBL bays from config"""
        depot_config = self.config.get_depot_config()
        return depot_config['ibl_bays']
    
    @property
    def WASH_BAYS(self) -> List[str]:
        """Get washing bays from config"""
        depot_config = self.config.get_depot_config()
        return depot_config['washing_bays']
    
    @property
    def ADVERTISERS(self) -> List[str]:
        """Get advertisers list from config"""
        return self.config.get_advertisers()
    
    @property
    def UNAVAILABLE_REASONS(self) -> List[str]:
        """Get unavailable reasons from config"""
        return self.config.get_unavailable_reasons()
    
    def generate_route(self, route_name: str = "Aluva-Pettah Line") -> Route:
        """Generate metro route with stations"""
        # Get route configuration
        route_config = self.config.get_route_config(route_name)
        route_defaults = self.config.get_route_defaults()
        
        stations = []
        total_distance = route_config.get('total_distance_km', 25.612)
        station_names = route_config['stations']
        
        for i in range(self.num_stations):
            distance = (total_distance / (self.num_stations - 1)) * i if self.num_stations > 1 else 0
            station = Station(
        ...
        return Route(
            route_id=route_config.get('route_id', 'ROUTE-001'),
            name=route_name,
            stations=stations,
    ...
    def generate_depot_layout(self, depot_name: str = "Muttom_Depot") -> Dict:
        """Generate depot bay layout"""
        depot_config = self.config.get_depot_config(depot_name)
        
        return {
            "depot_id": depot_config['depot_id'],
            "depot_name": depot_name,
            "location": depot_config['location'],
            "stabling_bays": depot_config['stabling_bays'].copy(),
            "ibl_bays": depot_config['ibl_bays'].copy(),
            "wash_bays": depot_config['washing_bays'].copy()
        }
